...
import { DuplicateRollDialog } from "./DuplicateRollDialog";
...
export function StudentJoinForm({ initialCode = "", onJoinSuccess }: StudentJoinFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === "DUPLICATE_ROLL_NUMBER" || err.message?.includes("already")) {
        setIsDuplicateModalOpen(true);
      } else {
        toast.error(err.message || "Failed to join election.");
      }
    } finally {
      setIsSubmitting(false);
    }
